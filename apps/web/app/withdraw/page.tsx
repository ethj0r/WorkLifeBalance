...
function Form({ onSubmit }: { onSubmit: () => void }) {
  return <Card className="auth-card">
        <div className="rounded-2xl bg-green-700 p-6 text-green-50">
            <div className="eyebrow !text-green-50/70">Saldo tersedia</div>
            <div className="figure mt-2 text-5xl font-medium leading-none">{formatIDR(user.balance)}</div>
            <div className="mt-2 text-sm text-green-50/70">Siap ditarik ke e-wallet</div>
        </div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Nominal">
                <TextInput defaultValue={formatIDR(user.balance)} />
            </Field>
        </div>
        <Button size="lg" className="mt-6" leftIcon={<Wallet className="h-4 w-4" />} onClick={onSubmit}>Tarik sekarang
        </Button>
    </Card>;
}
function Success() {
    return <Card className="auth-card py-16 text-center">
        ...
        <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-ink-500">Dana akan masuk ke e-wallet Anda dalam 1×24 jam.</p>
    </Card>; }
