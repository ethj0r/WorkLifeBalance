...
import { findOrCreateProfile, supabaseAvailable } from "@/lib/supabase/client";
import {
  clearPendingPhone,
  getPendingPhone,
  DEMO_USER,
...
const OTP_STUB = "123456";

export default function OTPPage() {
  const router = useRouter();
  const params = useSearchParams();
  const phone = params.get("phone") || getPendingPhone() || "812 3456 7890";
  ...
  async function onOTPSuccess() {
    ...
    try {
      if (supabaseAvailable) {
        const profile = await findOrCreateProfile(phone);
        if (profile) {
          setSession({
            id: profile.id,
            phone: profile.phone,
  ...
  function handle(i: number, value: string) {
    ...
    if (next.join("") === OTP_STUB) {
      setTimeout(() => onOTPSuccess(), 350);
    }
  }
          ...
          <div className="mt-8 grid grid-cols-6 gap-3">
            {code.map((v, i) => (
              <input
                ...
                className={`h-16 rounded-[14px] border bg-white text-center font-display text-3xl font-semibold outline-none transition disabled:opacity-50
                  ${v ? "border-green-600 shadow-[0_0_0_3px_rgba(47,104,64,.18)]" : "border-ink-200"}`}
              />
            ))}
          </div>
          {loading && (
            <p className="mt-4 text-center text-sm text-green-700 animate-pulse">
              Menyiapkan akun…
            </p>
          )}
          <div className="mt-6 text-center text-sm text-ink-500">
            {timer > 0
              ? <>Kirim ulang dalam <b>{timer}s</b></>
              : <button onClick={() => setTimer(60)} className="font-semibold text-green-700">Kirim ulang OTP</button>
            }
          </div>
          <div className="mt-5 rounded-lg bg-earth-50 p-3 text-center text-xs text-earth-700">
            Demo: ketik <b>123456</b>
          </div>
        </Card>
      </section>
    </main>
